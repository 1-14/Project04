...
void SM3_dealwith_MessageBlock(SM3::SM3Context* context)
{
	...
	if (i < 12) {
		...
		if (IsLittleEndian())	Reverse_word(&W[i + 4]);
	}
	else {
	...
	unsigned int messageLen, unsigned char digest[SM3_HASH_SIZE])
{
	...
	if (IsLittleEndian())
		for (i = 0; i < 8; i++)
			Reverse_word(context.intermediateHash + i);
...
int main() {
	char p[] = "lyb.txt";
	cout << "创建文件:" << p << endl;
	cout << "文件大小:2^10KB" << endl;
	CreatTxt(p, MAX_NUM);
	std::vector<uint32_t> hash_result;
	hash_result = SM3::call_hash_sm3(p);
	cout << "加密结果：" ;
	for (int i = 0; i < 32; i++) {
